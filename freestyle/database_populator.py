...
#
    with gzip.open(dataset,mode="rt") as data_file:
        all_data = ujson.load(data_file)


    print("Unzipped and returning")
...
def sessionsPopulator(sessions):
    ...
    for sid,session in tqdm(sessions.items()):
        ...
        try:
            word_keys = list(session["Words"].keys())
            wk = sorted(word_keys,key=float)
            words = []
            for word in wk:
                words.append(session["Words"][word])
            session_data["transcript_array"] = words
...
def usersPopulator(users):
    ...
    for uniqueid, data in tqdm(users.items()):
        user_data = {}
