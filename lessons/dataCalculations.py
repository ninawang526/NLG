...
def getData(data_list, version, high, low=0, std=0):
    if version == "none" or len(data_list) == 0:
        return {}
...
# Calculates average (scaled by duration of sentence), and standard deviation
...
# location_summary is filled with the list index of the significant group, so this
# replaces the index with the actual value
def fillInValues(location_summary, groups):
    summary = {}
    ...
    for spot in times:
        ...
        trend = groups[index]["trend"]
        val = groups[index]["value"]

        summary[spot] = {"trend":trend, "val":val}
...
def getLocationSummary(data_list, trend):
    ...
    if isEverywhere(groups, total_duration, trend):
        ...
        location_summary = putInTimeContext(groups, trend, total_duration)
        if location_summary["split"] == "thirds":
            location_summary = clean(location_summary["summary"])

        location_summary = fillInValues(location_summary, groups)
...
# Splits up list into even-sized blocks of time
